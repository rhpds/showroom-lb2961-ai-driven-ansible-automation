...
[quote]
⚠️ Reminder that the way we purposely broke httpd with the *❌ Break Apache* Job Template was by inserting *InvalidDirectiveHere* into the configuration file and restarting the service.  This crashed httpd service as you can see by running the *systemctl status httpd.service* Linux command.
...
[quote]
🎉 You have used Ansible Automation Platform in an AIOps workflow to automatically create an Ansible Job Template to fix your issue.  This has created an Ansible Job Template called *🔧✅ Execute HTTPD Remediation*.  You could also add this to the workflow, but it might be another natural stopping area where you can schedule this job template during an allowed change window.  For example, some organization might only want to apply changes to production servers on Sundays at 3AM.
...
[quote]
⚠️ A limit in Ansible Automation Platform is equivlanet to `--limit` with the `ansible-playbook` command.  The limit specifies the host or hosts you want to run the Ansible Job on versus the entire inventory, effectively *limiting* it to only running on those host(s).
